...
# Design is null so you can design it
## Example Usage:
## # Use defaults (used field data sample proportions: 296, 568, 284, 296, 556)
## sim1 <- simulate_roof(beta0 = -0.3, beta1 = 0.015, sd0 = 1.0, sd1 = 0.008, rho = -0.2)
##
## # Custom sample sizes
## sim2 <- simulate_roof(
##   beta0 = -0.3, beta1 = 0.015, sd0 = 1.0, sd1 = 0.008, rho = -0.2,
##   n_per_site = c(100, 150, 100, 200, 150)  # Smaller sample sizes
## )
##
## # Balanced design
## sim3 <- simulate_roof(
##   beta0 = -0.3, beta1 = 0.015, sd0 = 1.0, sd1 = 0.008, rho = -0.2,
##   n_per_site = c(400, 400, 400, 400, 400)  # Equal sample sizes
## )
##
## # Full customization (different sample sizes and year ranges per site)
## sim4 <- simulate_roof(
##   beta0 = -0.3, beta1 = 0.015, sd0 = 1.0, sd1 = 0.008, rho = -0.2,
##   n_per_site = c(150, 250, 100, 300, 200),
##   year_min = c(1940, 1950, 1930, 1960, 1980),  # Site-specific min years
##   year_max = c(1990, 2000, 1980, 2010, 2020)   # Site-specific max years
## )
...
simulate_roof <- function(beta0, beta1, sd0, sd1, rho,
                          ...
                          n_per_site = c(296, 568, 284, 296, 556),  # observations per site (length 5)
                          year_min = rep(1924, 5),      # minimum year for each site (length 5)
                          year_max = rep(2024, 5),      # maximum year for each site (length 5)
                          seed = NULL,
  ...
  stopifnot(is.numeric(beta0), is.numeric(beta1),
            ...
            is.numeric(n_per_site), length(n_per_site) == 5, all(n_per_site > 0),
            is.numeric(year_min), length(year_min) == 5,
            is.numeric(year_max), length(year_max) == 5,
            all(year_min <= year_max))

  # 1) Build or validate design ---------------------------------------------
  if (is.null(design)) {
    # Use fixed site names
